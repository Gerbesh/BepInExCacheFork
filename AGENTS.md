...
## Прогресс
- [2026-01-11] Переработан механизм валидации state cache Jotunn: добавлена проверка версии кеша и fingerprint при загрузке, улучшено логирование причин инвалидации. Создан ItemDataManagerGuard для перехвата ошибок при восстановлении поломанных данных — при обнаружении несовместимости state cache автоматически инвалидируется и пересчитывается на следующем запуске. Возвращена CacheState=true с защитой от ошибок. Готовность: 97% (state-cache теперь с auto-recovery).
- [2026-01-11] Реализован сверхранний перехват Jewelcrafting в BepInEx.Bootstrap.Entrypoint: подписка на AppDomain.CurrentDomain.AssemblyLoad при инициализации Bootstrap, при обнаружении загрузки Jewelcrafting немедленно инициализируется JewelcraftingNullSafePatcher через reflection. Это позволяет патчить EnsureDropCache ДО статических конструкторов и инициализации класса. Готовность: 99%.
- [2026-01-11] Полное обновление документации репо в соответствии со статусом 99% готовности: обновлены README.md (расширенная структура с таблицами и примерами), FAQ.md (50+ Q&A по диагностике), BUILDING.md (подробные инструкции сборки для всех платформ), CONTRIBUTING.md (PR workflow, правила кода, обновление AGENTS.md), cache.cfg (полное описание всех 50+ опций с примерами). Документация теперь полностью актуальна и может служить reference для новых разработчиков. Готовность: 99%.
- [2026-01-11] Усилен JewelcraftingNullSafePatcher для обработки случаев БЕЗ КЕША (первый запуск в мире): при первой ошибке EnsureDropCache теперь логируется предупреждение (может быть XUnity проблема или первый запуск) и разрешается базовый дроп без обогащения. На следующий успешный вызов результат кешируется. Создан дополнительный модуль JewelcraftingMatchLocalizedItemGuard для будущих оптимизаций matchLocalizedItem функции. Готовность: 99%.
- [2026-01-11] Адаптирован JewelcraftingNullSafePatcher под сохранение лута: добавлено кеширование успешных результатов EnsureDropCache по типу объекта. При повторных вызовах используется кешированный результат вместо пересчёта, что предотвращает повторные NRE от XUnity несоответствий. Готовность: 98%.
- [2026-01-11] Исправлен JewelcraftingNullSafePatcher: финализатор теперь возвращает null вместо исключения, чтобы ПОДАВИТЬ NRE в Jewelcrafting.EnsureDropCache. Это предотвращает бесконечный цикл убийства моба при XUnity несоответствиях. Готовность: 98%.
- [2026-01-11] Переработан JewelcraftingNullSafePatcher: исправлена точка патчирования с DoDrop на EnsureDropCache (где реально происходит NRE). Финализатор теперь гарантирует логирование ошибок и предотвращение бесконечного цикла при XUnity несоответствиях. Готовность: 98%.
- [2026-01-11] Добавлен JewelcraftingNullSafePatcher: защита от бесконечного цикла убийства моба при несоответствии локализованных имён предметов (XUnity AutoTranslator). Патч отключает XUnity на время DoDrop и блокирует глубокую рекурсию (>5 уровней). Готовность: 98%.
- [2026-01-11] Отключен патч JewelcraftingLocalizationGuard: исправлена сигнатура LocalizePrefix (добавлен __instance параметр) и отключено в cache.cfg по умолчанию. Причина: отключение Localization.Localize во время EnsureDropCache вызывает NRE в Jewelcrafting.LootSystem.EquipmentDrops.DoDrop при отбросе лута мобами. Готовность: 98%.
...
- [2026-01-10] Начата реализация FR-10: добавлены опции `ExtractedAssets` в `cache.cfg`, реализован extracted-кеш AssetBundle через `AssetBundle.RecompressAssetBundleAsync` (LZ4/Uncompressed), патчи `AssetBundle.LoadFromFile*` для перенаправления на extracted-кеш и очистка extracted_assets при ValidateStrict. Готовность: 98%.
- [2026-01-10] FR-10 доведён до рабочей сборки net35: перепаковка/копирование бандлов вынесены в reflection-обёртку (без зависимости на `UnityEngine.BuildCompression` при компиляции), добавлены статистика hit/miss для `LoadFromFile*` и фоновая прогревка ОС-кэша extracted-бандлов; DLL собраны и развернуты в Valheim и `runtime/`. Готовность: 99%.
- [2026-01-10] Исправлен FR-10 extracted-кеш: устранён `Ambiguous match` при Harmony-патче `AssetBundle.LoadFromFile*` (prefix теперь выбирается по сигнатуре через MethodInfo), extracted-патчи должны подключаться и перенаправлять загрузку на `extracted_assets`. Готовность: 99%.
- [2026-01-10] FR-10 усилен: добавлен on-demand extracted-кеш по реальным загрузкам (`AssetBundle.LoadFromFile*` → enqueue внешних путей, `AssetBundle.LoadFromMemory*` → кеш по content-hash), расширен формат `extracted_manifest.txt` (опциональный `contentHashHex`), добавлен фоновый builder `BuildPendingAsync` и более информативные логи miss с полным путём. Готовность: 99%.
- [2026-01-10] FR-10 расширен под Jotunn/embedded bundles: добавлены патчи `AssetBundle.LoadFromStream*`/`LoadFromStreamAsync*` с захватом стрима в файл, вычислением content-hash и восстановлением через `LoadFromFile*` из extracted-кеша; добавлен enqueue `EnqueueObservedContentFile` для стримов без удержания в памяти. Готовность: 99%.
- [2026-01-10] Исправлен cache-hit для extracted stream-кеша: учтён `offset`/`Position` при хешировании `LoadFromStream*`, временные файлы `__incoming_stream/tmp_*.bin` удаляются при hit и после успешной перепаковки, чтобы 2-й запуск не «переписывал» кеш случайными именами. Готовность: 99%.
- [2026-01-10] FR-10 оптимизирован для ускорения 2+ запуска: добавлен быстрый путь для embedded AssetBundle через `Assembly.GetManifestResourceStream` (детерминированный ключ по `ModuleVersionId+resourceName`, отдача `FileStream` из `__resource` без чтения DLL), а `LoadFromStream*` при уже extracted `FileStream` перенаправляется на `LoadFromFile*`. Готовность: 99%.
- [2026-01-10] Улучшен cache-hit для embedded bundles без `__resource`: добавлен `extracted_resource_map.txt` (resourceKey→contentHash), на cache-hit `GetManifestResourceStream` теперь отдаёт `FileStream` из `__content` по маппингу; если маппинга нет — он «обучается» на лету (хешируется при чтении Unity без двойного чтения), чтобы следующий запуск реально шел с диска. Готовность: 99%.
- [2026-01-10] Исправлена причина отсутствия `extracted_resource_map.txt`: `GetManifestResourceStream` у Mono вызывается через внутренние наследники `Assembly`, поэтому патч теперь ставится на все конкретные типы `Assembly` в `mscorlib` (а не только на базовый абстрактный метод); добавлен лог подключения патчей. Готовность: 99%.
- [2026-01-10] Добавлен единый сбор метрик/таймингов (`CacheMetrics`) и сводка на меню/по таймауту (`CacheSummaryReporter`): агрегируются времена CacheManager/Fingerprint/BuildAndDump, Valheim ObjectDB и Jotunn‑этапы, а также печатается сводка размера extracted‑кеша и количества строк в `extracted_resource_map.txt`. Готовность: 99%.
- [2026-01-10] Оптимизирован fingerprint: добавлен `FingerprintMode=Fast|Strict` (по умолчанию Fast — размер+mtime без чтения содержимого файлов) и кеширование fingerprint внутри процесса (чтобы `TryLoadCache` не считал его дважды). Это убирает десятки секунд накладных расходов на 100+ модах. Готовность: 99%.
- [2026-01-10] Исправлена «вечная» инвалидация кеша на 2-м запуске: `BuildAndDump` теперь пересчитывает fingerprint заново (после того как моды могли распаковать файлы в `plugins/`), пишет `fingerprint_snapshot.txt` и при mismatch логирует diff по снимку; тяжёлый `BuildAllAsync` для extracted-кеша больше не стартует при наличии `extracted_manifest.txt`, а hit-статистика расширена на `LoadFromMemory*`/`LoadFromStream*`. Готовность: 99%.
- [2026-01-10] Устранена ложная инвалидация по `AutoTranslatorConfig.ini`: в `FingerprintMode=Fast` этот файл теперь учитывается по content-hash (а не по `LastWriteTimeUtc`), потому что XUnity может обновлять timestamp без изменения содержимого, что ломало cache-hit и провоцировало пересборку/очистку extracted-кеша. Готовность: 99%.
- [2026-01-10] FR-10 расширен: добавлен перехват `UnityEngine.Networking.UnityWebRequestAssetBundle.GetAssetBundle*` — если url указывает на локальный файл, он перенаправляется на extracted-кеш (CRC сбрасывается в 0). Для диагностики добавлено профилирование «инициализации плагинов» (время `Awake` при `ManagerObject.AddComponent`) с выводом топа по суммарному времени при `VerboseDiagnostics=true`. Готовность: 99%.
- [2026-01-10] Улучшена практическая пригодность диагностики и стабильность extracted-кеша: в логах extracted hit/miss теперь добавляется контекст текущего плагина (`ctx=name (guid)`) во время его `Awake`, а захваченные `__incoming_stream/tmp_*.bin` удаляются при cache-hit miss (чтобы кеш не раздувался от одноразовых стримов) и дополнительно чистятся по таймауту. Для ускорения restore-mode ослаблена эвристика fast-path `ObjectDB.CopyOtherDB` (без требования по количеству рецептов). Готовность: 99%.
- [2026-01-11] Усилен restore-mode для `ObjectDB.CopyOtherDB`: fast-path теперь может применяться не только на «поздней» стадии (2000+ items), но и на раннем вызове `FejdStartup.SetupObjectDB` (проверка по stacktrace + признаки заполненности otherDb). Цель — убрать стабильные ~6–7с на `CopyOtherDB` на cache-hit. Готовность: 99%.
- [2026-01-10] Ускорен fingerprint в Fast-режиме: добавлен `FingerprintParallelism` (ThreadPool) для параллельного чтения метаданных/контент-хеша без изменения порядка/стабильности итогового хеша. Готовность: 98%.
- [2026-01-10] Добавлена фоновая прогревка ОС-кэша extracted ассетов в отдельном потоке (опционально `WarmupThreaded`, с лимитами `WarmupMaxMB`/`WarmupMaxSeconds`), чтобы не нагружать главный поток Unity. Готовность: 98%.
- [2026-01-10] Добавлен экспериментальный режим отложенной инициализации плагинов на cache-hit (whitelist/blacklist, триггер из Valheim-меню через `FejdStartup.Start`), чтобы можно было уменьшать время до меню без вмешательства в логику модов по умолчанию. Готовность: 98%.
- [2026-01-11] Отключен fast-skip `ObjectDB.CopyOtherDB`, если метод пропатчен сторонними Harmony-патчами (например, Jotunn.ItemManager), чтобы убрать NRE на cache-hit при ранней инициализации. Готовность: 98%.
- [2026-01-11] Включён по умолчанию санитайз секций/ключей конфигов (SanitizeInvalidConfigChars=true), чтобы моды с запрещёнными символами не падали на ArgumentException. Готовность: 98%.
