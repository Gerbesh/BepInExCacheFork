...
## Прогресс
- [2026-01-11] Исправлена проблема с применением JewelcraftingNullSafePatcher: добавлен ранний перехват при загрузке сборки Jewelcrafting в OnPluginAssemblyLoaded. Теперь патч подключается ДО того, как класс загружается, вместо попытки патчировать уже загруженный класс. Это исправляет ситуацию, когда Harmony не может перехватить статический конструктор после инициализации. Готовность: 99%.
